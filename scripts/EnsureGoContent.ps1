...
# NOTE: The current version of this script is a little bit slow, maybe we can improve the
# performance later.
...
$typesFiles = Get-ChildItem -Path $TargetPath -Recurse -Include "*types*.go"
$helpersFiles = Get-ChildItem -Path $TargetPath -Recurse -Include "*helpers*.go"
$methodsFiles = Get-ChildItem -Path $TargetPath -Recurse -Include "*methods*.go"
$constantsFiles = Get-ChildItem -Path $TargetPath -Recurse -Include "*constants*.go"
$varsFiles = Get-ChildItem -Path $TargetPath -Recurse -Include "*vars*.go"
$handlersFiles = Get-ChildItem -Path $TargetPath -Recurse -File | Where-Object {
